...
void usage() {
    *env << "Usage: " << progName
         ...
         << " [-w <width> -h <height>] [-f <frames-per-second>] [-y] [-H] [-Q [<measurement-interval>]] [-F <filename-prefix>] [-b <file-sink-buffer-size>] [-B <input-socket-buffer-size>] [-I <input-interface-ip-address>] [-m] [<url>|-R [<port-num>]] (or "
         << progName << " -o [-V] <url>)\n";
    shutdown();
}
...
int main(int argc, char **argv) {
        ...
        switch (opt[1]) {
            ...
            case 'R': {
                ...
                if (argc > 2 && argv[2][0] != '-') {
                    ...
                    if (sscanf(argv[2], "%hu", &handlerServerForREGISTERCommandPortNum) == 1 &&
    ...
    if (!((argc == 2 && !createHandlerServerForREGISTERCommand) ||
          (argc == 1 && createHandlerServerForREGISTERCommand)))
        usage();
                ...
                if (subsession->rtpSource() != NULL) {
                    ...
                    if (socketInputBufferSize > 0 || fileSinkBufferSize > curBufferSize) {
                        ...
                        if (socketInputBufferSize >
                            0) { // The user explicitly asked for the new socket buffer size; announce it:
...
void continueAfterPLAY(RTSPClient *, int resultCode, char *resultString) {
    ...
    if (duration > 0) {
        ...
        sessionTimerTask = env->taskScheduler().scheduleDelayedTask(uSecsToDelay, (TaskFunc *) sessionTimerHandler,
                                                                    (void *) NULL);
    }
            ...
            if (curQOSRecord != NULL) {
                ...
                *env << "packet_loss_percentage_min\t" << 100 * curQOSRecord->packet_loss_fraction_min << "\n";
                double packetLossFraction = numPacketsExpected == 0 ? 1.0
                                                                    : 1.0 -
                                                                      numPacketsReceived / (double) numPacketsExpected;
